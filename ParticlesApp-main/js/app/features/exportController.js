 ...
 */
export async function exportImage(app, format, scale, { transparentBackground = false } = {}) {
    ...
    if (wasAnimating && typeof app.stopAnimation === 'function') {
        app.stopAnimation();
    }
    ...
    try {
        ...
        const renderSettings = {
            ...
            chromaticShift: app.settings.chromaticShift,
        ...
        // Close modal
        if (app.exportModal) app.exportModal.classList.remove('active');
    } catch (err) {
        console.error(err);
        alert('Image export failed.');
    } finally {
        // Restore canvas size
        if (app.renderer && typeof app.renderer.resizeTo === 'function') {
            app.renderer.resizeTo(originalWidth, originalHeight);
        }
        if (typeof app.handleResize === 'function') {
            app.handleResize();
        }
        if (wasAnimating && typeof app.startAnimation === 'function') {
            app.startAnimation();
        }
    }
}
...
export async function exportVideo(app, { format = 'webm', scale = 2, fps = 30 } = {}) {
    ...
    const hasImageItems = isMixedSource
        ...
        : isImageSource;
    const existingTargets = app.logoSequence ? app.logoSequence.gpuTargets : null;
    const needBuild = !existingTargets ||
        existingTargets.count !== desiredCount ||
        !Array.isArray(existingTargets.textures) ||
        existingTargets.textures.length === 0 ||
        (hasImageItems && (!Array.isArray(existingTargets.colorTextures) || existingTargets.colorTextures.length === 0));
    ...
    const targets = app.logoSequence ? app.logoSequence.gpuTargets : null;
