...
function getImageDepthOptions(app) {
    return {
        depthMapEnabled: !!(app && app.settings && app.settings.imageDepthMapEnabled),
        depthMapStrength: clamp01(app?.settings?.imageDepthMapStrength ?? 1),
        depthMapInvert: !!(app && app.settings && app.settings.imageDepthMapInvert)
    };
}
