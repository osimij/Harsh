...
export class ParticleForge {
    ...
    /**
     * Load preset settings from window.PARTICLE_PRESET or window.PARTICLE_PRESET_URL
     */
    loadPresetSettings() {
        if (window.PARTICLE_PRESET) {
            // Inline preset object
            try {
                this.importSettings(window.PARTICLE_PRESET);
                logger.log('Loaded inline preset settings');
            } catch (e) {
                logger.warn('Failed to load inline preset:', e);
            }
        } else if (window.PARTICLE_PRESET_URL) {
            // Load from URL
            fetch(window.PARTICLE_PRESET_URL)
                .then(r => {
                ...
                .then(settings => {
                    ...
                    logger.log('Loaded preset from URL:', window.PARTICLE_PRESET_URL);
                })
                .catch(e => {
                    logger.warn('Failed to load preset from URL:', e);
                });
        }
    }
    ...
    getImageDepthMapOptions() {
        return {
            depthMapEnabled: !!(this.settings && this.settings.imageDepthMapEnabled),
            depthMapStrength: clamp01(this.settings?.imageDepthMapStrength ?? 1),
            depthMapInvert: !!(this.settings && this.settings.imageDepthMapInvert)
        };
    }
    ...
    isUploadTokenStale(uploadToken) {
        return Number.isFinite(uploadToken) && uploadToken !== this._uploadToken;
    }
    ...
    async handleFiles(files) {
        ...
        try {
            ...
            if (!filtered.length) {
                alert('No supported files found. Upload SVG, PNG, JPG, or WEBP files.');
                return;
            }
            ...
            if (filtered.length === 1) {
                ...
                if (only.type === 'image') {
                    await this.processImage(only.imageInfo, { uploadToken });
                } else {
                    this.processSVG(only.svgString, { uploadToken });
                }
            ...
            if (hasSvg && hasImage) {
                await this.processMixedSequence(filtered, { uploadToken });
            } else if (hasImage) {
                const images = filtered.map((item) => item.imageInfo);
                await this.processImageSequence(images, { uploadToken });
            } else {
                const svgStrings = filtered.map((item) => item.svgString);
                await this.processSVGSequence(svgStrings, { uploadToken });
            }

            // Allow re-uploading the same file (some browsers won't fire change if value unchanged)
            if (this.svgInput) this.svgInput.value = '';
        } catch (error) {
            if (this.isUploadTokenStale(uploadToken)) return;
     ...
     */
    processSVG(svgString, { uploadToken = null, skipHighDensityGPU = false } = {}) {
        try {
            if (this.isUploadTokenStale(uploadToken)) return;
            ...
            if (!skipHighDensityGPU && desiredCount >= 200000 && this.gpu.supported && rendererHasGPU) {
                ...
                this.buildGPUTargetsForSingle(sanitized).catch((e) => {
                    logger.warn(e);
                    if (this.isUploadTokenStale(uploadToken)) return;
                    // If GPU target building fails, transparently fall back to CPU/raster so the logo still appears.
                    this.processSVGRasterFallback(sanitized, { uploadToken }).catch((fallbackErr) => {
                        if (this.isUploadTokenStale(uploadToken)) return;
                        console.error(fallbackErr);
                        alert('Error processing SVG after GPU fallback: ' + fallbackErr.message);
                    });
                });
                return;
            }
            ...
            if (!hasVectorPaths(this.svgData)) {
                this.processSVGRasterFallback(sanitized, { uploadToken }).catch((e) => {
                    if (this.isUploadTokenStale(uploadToken)) return;
                    console.error(e);
     ...
     */
    async processImage(imageInfo, { uploadToken = null } = {}) {
        try {
            if (this.isUploadTokenStale(uploadToken)) return;
            this.stopLogoSequence();
            ...
            if (chromaStrengthSlider) chromaStrengthSlider.disabled = String(this.settings.colorMode || '') !== 'chromatic';

            const desiredCount = this.getDesiredParticleCount({ type: 'image' });
            const rendererHasGPU = !!(this.renderer && this.renderer.gpuProgram);
     ...
     */
    async processImageSequence(imageInfos, { uploadToken = null } = {}) {
        try {
            if (this.isUploadTokenStale(uploadToken)) return;
            this.stopLogoSequence();
            this.currentSourceType = 'image';
     ...
     */
    async processMixedSequence(items, { uploadToken = null } = {}) {
        try {
            if (this.isUploadTokenStale(uploadToken)) return;
            this.stopLogoSequence();
            ...
            const prepared = [];
            let skippedInvalidSvgs = 0;
            for (let i = 0; i < list.length; i++) {
                const item = list[i];
                if (item && item.type === 'image') {
                    prepared.push({ type: 'image', imageInfo: item.imageInfo });
                    continue;
                }
                const rawSvg = item && item.svgString ? item.svgString : '';
                try {
                    const { sanitized, svgData } = sanitizeAndParse(this.svgParser, rawSvg);
                    prepared.push({ type: 'svg', svgString: sanitized, svgData });
                } catch (_err) {
                    skippedInvalidSvgs++;
                }
            }
            ...
            if (!prepared.length) {
                throw new Error('No valid files found in the mixed upload.');
            }

            const hasSvg = prepared.some((item) => item && item.type === 'svg');
            const hasImage = prepared.some((item) => item && item.type === 'image');

            this.currentSourceType = prepared[0] && prepared[0].type ? prepared[0].type : 'svg';
            this.currentSvgString = null;
            this.currentImage = null;
            this.svgData = null;

            const logoCount = prepared.length;
            this.resetParticleIconAssignments(logoCount);
            if (typeof this.refreshParticleIconUI === 'function') {
                this.refreshParticleIconUI();
            }
            ...
            for (let i = 0; i < logoCount; i++) {
                ...
                if (item && item.type === 'image') {
                    ...
                    const sanitized = item && item.svgString ? item.svgString : '';
                    const svgData = item && item.svgData ? item.svgData : null;
            ...
            if (poolCount >= 200000 && this.gpu.supported && rendererHasGPU) {
                ...
                if (skippedInvalidSvgs > 0) {
                    alert(`Some files were skipped: ${skippedInvalidSvgs} invalid SVG file(s).`);
                }
                return;
            }
     ...
     */
    async processSVGRasterFallback(svgString, { uploadToken = null } = {}) {
        if (this.isUploadTokenStale(uploadToken)) return;
        const desiredCount = this.getDesiredParticleCount({ type: 'svg' });
        const rasterSize = desiredCount >= 60000 ? 2048 : 1024;
     ...
     */
    async processSVGSequence(svgStrings, { uploadToken = null, skipHighDensityGPU = false } = {}) {
        try {
            if (this.isUploadTokenStale(uploadToken)) return;
            this.stopLogoSequence();
            this.currentSourceType = 'svg';
            ...
            const { sanitizedStrings, svgDatas, failedIndexes = [] } = sanitizeAndParseMany(this.svgParser, svgStrings);
            const valid = [];
            const skippedInitial = new Set(Array.isArray(failedIndexes) ? failedIndexes : []);

            for (let i = 0; i < sanitizedStrings.length; i++) {
                const sanitized = sanitizedStrings[i];
                const svgData = svgDatas[i];
                if (!sanitized || !svgData) {
                    skippedInitial.add(i);
                    continue;
                }
                valid.push({ sanitized, svgData });
            }
            ...
            if (skippedInitial.size > 0) {
                skippedMessages.push(`${skippedInitial.size} invalid SVG file(s)`);
            }

            // High-density path: GPU-only sequencing (targets built async, no CPU point clouds)
            if (!skipHighDensityGPU && desiredCount >= 200000 && this.gpu.supported && rendererHasGPU) {
                const validStrings = valid.map((entry) => entry.sanitized);
                const validDatas = valid.map((entry) => entry.svgData);

                this.currentSvgString = validStrings[0] || this.currentSvgString;
                const logoCount = validStrings.length;
                this.resetParticleIconAssignments(logoCount);
                if (typeof this.refreshParticleIconUI === 'function') {
                    this.refreshParticleIconUI();
                }
                ...
                this.logoSequence = this.state.logoSequence = {
                    active: validStrings.length > 1,
                    sourceType: 'svg',
                    svgStrings: [...validStrings],
                    svgDatas: validDatas,
                    imageSources: [],
                    // Placeholder clouds for length only (GPU sequencing uses textures)
                    pointClouds: new Array(validStrings.length).fill([]),
                    logoIds: validStrings.map((s) => this.hashString32(s)),
                    countRatios: new Array(validStrings.length).fill(1),
                    gpuTargets: null,
                    index: 0,
                    transition: null,
                    holdTimer: 0
                ...
                this.uploadZone.classList.add('hidden');
                this.updateParticleCount();
                this.buildGPUTargetsForSequence(validStrings).catch(async (e) => {
                    logger.warn(e);
                    if (this.isUploadTokenStale(uploadToken)) return;
                    // Retry through the non-GPU path so complex SVGs still produce visible particles.
                    try {
                        await this.processSVGSequence(validStrings, {
                            uploadToken,
                            skipHighDensityGPU: true
                        });
                    } catch (fallbackErr) {
                        if (this.isUploadTokenStale(uploadToken)) return;
                        console.error(fallbackErr);
                        alert('Error processing SVG sequence after GPU fallback: ' + fallbackErr.message);
                    }
                });

                if (skippedMessages.length) {
                    alert(`Some files were skipped: ${skippedMessages.join(', ')}.`);
                }
                return;
            }
            ...
            const finalStrings = [];
            const finalDatas = [];
            let skippedSampling = 0;

            for (const entry of valid) {
                if (this.isUploadTokenStale(uploadToken)) return;

                const svgString = entry.sanitized;
                const svgData = entry.svgData;
                let points = [];

                if (svgData && hasVectorPaths(svgData)) {
                    const sampled = this.pointSampler.sample(svgData, desiredCount);
                    points = this.fitPointCount(sampled, desiredCount);
                } else {
                    const rasterSize = desiredCount >= 60000 ? 2048 : 1024;
                    // eslint-disable-next-line no-await-in-loop
                    const sampled = await this.rasterPointSampler.samplePoints(svgString, desiredCount, {
                        rasterSize,
                        seed: this.settings.transitionSeed,
                        edgeRatio: 0.6
                    });
                ...
                finalStrings.push(svgString);
                finalDatas.push(svgData);
            }
            ...
            if (skippedSampling > 0) {
                skippedMessages.push(`${skippedSampling} SVG file(s) with no usable particles`);
            }

            this.currentSvgString = finalStrings[0] || this.currentSvgString;
            const logoCount = finalStrings.length;
            this.resetParticleIconAssignments(logoCount);
            if (typeof this.refreshParticleIconUI === 'function') {
                this.refreshParticleIconUI();
            }
            ...
            this.logoSequence = this.state.logoSequence = {
                active: pointClouds.length > 1,
                sourceType: 'svg',
                svgStrings: [...finalStrings],
                svgDatas: finalDatas,
                imageSources: [],
                pointClouds,
                logoIds: finalStrings.map((s) => this.hashString32(s)),
                countRatios: new Array(pointClouds.length).fill(1),
                gpuTargets: null,
                index: 0,
                transition: null,
            ...
            if (this.settings && this.settings.fluidGPU) {
                ...
                this.buildGPUTargetsForSequence(finalStrings).catch((e) => logger.warn(e));
            }
    ...
    regenerateParticles() {
        ...
        if (this.logoSequence && this.logoSequence.active && this.logoSequence.svgDatas && this.logoSequence.svgDatas.length) {
            const svgDatas = Array.isArray(this.logoSequence.svgDatas) ? this.logoSequence.svgDatas : [];
            const svgStrings = Array.isArray(this.logoSequence.svgStrings) ? this.logoSequence.svgStrings : [];
            const rasterSize = desiredCount >= 60000 ? 2048 : 1024;
            const token = ++this._rasterRegenToken;

            Promise.all(svgDatas.map((data, idx) => {
                if (data && hasVectorPaths(data)) {
                    const sampled = this.pointSampler.sample(data, desiredCount);
                    const fitted = this.fitPointCount(sampled, desiredCount);
                    const withAura = this.applyEdgeAuraToLogoPoints(fitted, {
                        amount: this.settings.edgeAuraAmount,
                        spread: this.settings.edgeAuraSpread,
                        outlier: this.settings.edgeAuraOutlier
                    });
                    return Promise.resolve(withAura);
                }

                const svgString = svgStrings[idx] || '';
                if (!svgString) return Promise.resolve([]);

                return this.rasterPointSampler.samplePoints(svgString, desiredCount, {
                    rasterSize,
                    seed: this.settings.transitionSeed,
                    edgeRatio: 0.6
                }).then((sampled) => {
                ...
                const firstUsable = clouds.find((pts) => Array.isArray(pts) && pts.length) || [];
                ...
                const pointClouds = clouds.map((pts) => (Array.isArray(pts) && pts.length) ? pts : firstUsable);
                this.logoSequence.pointClouds = pointClouds;
                this.logoSequence.index = Math.max(0, Math.min(this.logoSequence.index, pointClouds.length - 1));
                this.logoSequence.transition = null;
                this.logoSequence.holdTimer = 0;
    ...
    resetSettings() {
        ...
        if (imageDepthToggle) imageDepthToggle.checked = !!this.settings.imageDepthMapEnabled;
        ...
        const imageDepthStrengthPct = Math.round(clamp01(this.settings.imageDepthMapStrength ?? 1) * 100);
        ...
        if (imageDepthBrightNearToggle) imageDepthBrightNearToggle.checked = !!this.settings.imageDepthMapInvert;
        const imageDepthDisabled = !(this.settings && this.settings.imageDepthMapEnabled);
        ...
        const chromaPct = Math.round(clamp01(this.settings.chromaticShift ?? 0.18) * 100);
    ...
    /**
     * Import settings from JSON and update UI/particles
     * @param {string|object} settingsJson - JSON string or object with settings
     */
    importSettings(settingsJson) {
        let newSettings;
        try {
            newSettings = typeof settingsJson === 'string'
        ...
        if (!newSettings || typeof newSettings !== 'object') {
            console.error('Settings payload is not an object.');
            return false;
        }
        ...
        // Apply density/point changes to the current source
        if (this.svgData || this.currentImage || (this.logoSequence && this.logoSequence.sourceType)) {
            this.regenerateParticles();
        } else {
            this.updateParticleCount();
        }

        // Trigger UI refresh event for controls to sync
     ...
     */
    async exportImage(format, scale, opts) {
        const rt = this.state && this.state.runtime;
        if (rt && rt.isExporting) return;
        if (rt) rt.isExporting = true;
        try {
            return await this.controllers.export.exportImage(format, scale, opts);
        } finally {
