...
export function setupControls({ appCompat, lifecycle }) {
    ...
    const imageDepthToggle = document.getElementById('image-depth-map');
    const imageDepthStrengthSlider = document.getElementById('image-depth-strength');
    const imageDepthStrengthValue = document.getElementById('image-depth-strength-value');
    const imageDepthBrightNearToggle = document.getElementById('image-depth-bright-near');

    const setImageDepthControlsDisabled = (disabled) => {
        if (imageDepthStrengthSlider) imageDepthStrengthSlider.disabled = disabled;
        if (imageDepthBrightNearToggle) imageDepthBrightNearToggle.disabled = disabled;
    };

    const hasImageSource = () => !!(app.currentImage ||
        (app.logoSequence && (app.logoSequence.sourceType === 'image' || app.logoSequence.sourceType === 'mixed')));

    if (imageDepthToggle) {
        imageDepthToggle.checked = !!app.settings.imageDepthMapEnabled;
        on(imageDepthToggle, 'change', (e) => {
            app.settings.imageDepthMapEnabled = !!e.target.checked;
            setImageDepthControlsDisabled(!app.settings.imageDepthMapEnabled);
            if (hasImageSource()) {
                app.regenerateParticles();
            }
        });
    ...
    if (imageDepthStrengthSlider) {
        const pct = Math.round(clamp01(app.settings.imageDepthMapStrength ?? 1) * 100);
        imageDepthStrengthSlider.value = String(pct);
        if (imageDepthStrengthValue) imageDepthStrengthValue.textContent = `${pct}%`;
    }

    if (imageDepthBrightNearToggle) {
        imageDepthBrightNearToggle.checked = !!app.settings.imageDepthMapInvert;
        on(imageDepthBrightNearToggle, 'change', (e) => {
            app.settings.imageDepthMapInvert = !!e.target.checked;
            if (hasImageSource()) {
                app.regenerateParticles();
            }
        });
    ...
    setImageDepthControlsDisabled(!(app.settings && app.settings.imageDepthMapEnabled));

    setupSlider('image-depth-strength', 100, (value) => {
        const pct = Math.max(0, Math.min(100, parseInt(value, 10) || 0));
        app.settings.imageDepthMapStrength = pct / 100;
        if (hasImageSource()) {
            app.regenerateParticles();
        }
    }, {
    ...
    const chromaStrengthSlider = document.getElementById('chromatic-strength');
    const chromaStrengthValue = document.getElementById('chromatic-strength-value');
    const setChromaticControlsDisabled = (disabled) => {
        if (chromaStrengthSlider) chromaStrengthSlider.disabled = disabled;
    };
    const applyChromaticStrengthLabel = (strength01) => {
        if (!chromaStrengthValue) return;
        const pct = Math.round(clamp01(strength01) * 100);
        chromaStrengthValue.textContent = `${pct}%`;
    };
    const setColorMode = (color) => {
        app.settings.colorMode = color;
        app.particleSystem.setColorOverride(color);
        setChromaticControlsDisabled(String(color || '') !== 'chromatic');
    };
    ...
    // Chromatic strength (0..40 => 0..0.40 shift)
    if (chromaStrengthSlider) {
        const strength01 = clamp01(app.settings.chromaticShift ?? 0.18);
        chromaStrengthSlider.value = String(Math.round(strength01 * 100));
        applyChromaticStrengthLabel(strength01);
    }
    setChromaticControlsDisabled(String(app.settings.colorMode || '') !== 'chromatic');

    setupSlider('chromatic-strength', 18, (value) => {
        const v = Math.max(0, Math.min(40, parseInt(value, 10) || 0));
        app.settings.chromaticShift = v / 100;
    }, {
        lifecycle,
        onInput: (value) => {
            applyChromaticStrengthLabel((parseInt(value, 10) || 0) / 100);
        }
    });
    ...
    function syncSettingsUI() {
        ...
        const logoDensity = (typeof app.getDensityForType === 'function')
            ? app.getDensityForType('svg')
            : (app.settings.logoDensity ?? app.settings.density ?? 15000);
        const imageDensity = (typeof app.getDensityForType === 'function')
            ? app.getDensityForType('image')
            : (app.settings.imageDensity ?? app.settings.density ?? 15000);
        ...
        if (typeof syncLegacyDensity === 'function') syncLegacyDensity();

        const sizeEl = document.getElementById('particle-size');
        const sizeValue = document.getElementById('size-value');
        if (sizeEl) sizeEl.value = String(app.settings.size);
        if (sizeValue) sizeValue.textContent = Number(app.settings.size).toFixed(1);

        const depthPct = Math.round((Number(app.settings.depthVariance) || 0) * 100);
        const depthEl = document.getElementById('depth');
        const depthValue = document.getElementById('depth-value');
        if (depthEl) depthEl.value = String(depthPct);
        if (depthValue) depthValue.textContent = depthPct + '%';

        if (imageDepthToggle) imageDepthToggle.checked = !!app.settings.imageDepthMapEnabled;
        const imageDepthStrengthPct = Math.round(clamp01(app.settings.imageDepthMapStrength ?? 1) * 100);
        if (imageDepthStrengthSlider) imageDepthStrengthSlider.value = String(imageDepthStrengthPct);
        if (imageDepthStrengthValue) imageDepthStrengthValue.textContent = imageDepthStrengthPct + '%';
        if (imageDepthBrightNearToggle) imageDepthBrightNearToggle.checked = !!app.settings.imageDepthMapInvert;
        const imageDepthDisabled = !(app.settings && app.settings.imageDepthMapEnabled);
        if (imageDepthStrengthSlider) imageDepthStrengthSlider.disabled = imageDepthDisabled;
        if (imageDepthBrightNearToggle) imageDepthBrightNearToggle.disabled = imageDepthDisabled;

        const glowPct = Math.round((Number(app.settings.glowIntensity) || 0) * 100);
        const glowEl = document.getElementById('glow');
        const glowValue = document.getElementById('glow-value');
        if (glowEl) glowEl.value = String(glowPct);
        if (glowValue) glowValue.textContent = glowPct + '%';

        const speedPct = Math.round((Number(app.settings.animationSpeed) || 0) * 100);
        const speedEl = document.getElementById('animation-speed');
        const speedValue = document.getElementById('speed-value');
        if (speedEl) speedEl.value = String(speedPct);
        if (speedValue) speedValue.textContent = speedPct + '%';

        const zoomPct = Math.round((Number(app.settings.zoom) || 1) * 100);
        const zoomEl = document.getElementById('zoom');
        const zoomValue = document.getElementById('zoom-value');
        if (zoomEl) zoomEl.value = String(zoomPct);
        if (zoomValue) zoomValue.textContent = zoomPct + '%';

        const sizeRandPct = Math.round((Number(app.settings.sizeRandom) || 0) * 100);
        const sizeRandomEl = document.getElementById('size-random');
        const sizeRandomValue = document.getElementById('size-random-value');
        if (sizeRandomEl) sizeRandomEl.value = String(sizeRandPct);
        if (sizeRandomValue) sizeRandomValue.textContent = sizeRandPct + '%';

        const sizeMin = Number(app.settings.sizeMin);
        const sizeMax = Number(app.settings.sizeMax);
        ...
        const opacityRandPct = Math.round((Number(app.settings.opacityRandom) || 0) * 100);
        const opacityRandomEl = document.getElementById('opacity-random');
        const opacityRandomValue = document.getElementById('opacity-random-value');
        if (opacityRandomEl) opacityRandomEl.value = String(opacityRandPct);
        if (opacityRandomValue) opacityRandomValue.textContent = opacityRandPct + '%';

        const opacityMinPct = Math.round((Number(app.settings.opacityMin) || 0) * 100);
        const opacityMaxPct = Math.round((Number(app.settings.opacityMax) || 0) * 100);
        ...
        if (squaresEnabledToggle) squaresEnabledToggle.checked = !!app.settings.squaresEnabled;
        const squareRatioPct = Math.round((Number(app.settings.squareRatio) || 0) * 100);
        if (squaresRatioSlider) {
            squaresRatioSlider.value = String(squareRatioPct);
            squaresRatioSlider.disabled = !app.settings.squaresEnabled;
        }
        if (typeof applySquaresRatioLabel === 'function') {
            applySquaresRatioLabel(app.settings.squareRatio);
        } else if (squaresValueLabel) {
            squaresValueLabel.textContent = squareRatioPct + '%';
        }

        if (edgeAuraToggle) edgeAuraToggle.checked = !!app.settings.edgeAuraEnabled;
        const edgeAuraAmountPct = Math.round(clamp01(app.settings.edgeAuraAmount ?? 0.12) * 100);
        const edgeAuraSpreadPct = Math.round(clamp01(app.settings.edgeAuraSpread ?? 0.12) * 100);
        const edgeAuraOutlierPct = Math.round(clamp01(app.settings.edgeAuraOutlier ?? 0.05) * 100);
        ...
        const edgeAuraDisabled = !(app.settings && app.settings.edgeAuraEnabled);
        ...
        if (focusToggle) focusToggle.checked = !!app.settings.focusEnabled;
        const focusRadiusPct = Math.round(Math.max(0.1, Math.min(1.2, Number(app.settings.focusRadius ?? 0.45))) * 100);
        const focusSoftnessPct = Math.round(clamp01(app.settings.focusSoftness ?? 0.35) * 100);
        const focusScatterPct = Math.round(Math.max(0, Math.min(2.5, Number(app.settings.focusScatter ?? 1.5))) * 100);
        ...
        const focusDisabled = !(app.settings && app.settings.focusEnabled);
        ...
        const particleIconsToggle = document.getElementById('particle-icons-enabled');
        if (particleIconsToggle) particleIconsToggle.checked = !!app.settings.particleIconEnabled;
        const particleIconRotation = document.getElementById('particle-icon-rotation');
        if (particleIconRotation) particleIconRotation.value = app.settings.particleIconRotate ? 'spin' : 'still';
        const particleIconColorMode = document.getElementById('particle-icon-color-mode');
        if (particleIconColorMode) particleIconColorMode.value = String(app.settings.particleIconColorMode || 'tint');
        if (typeof refreshParticleIconUI === 'function') refreshParticleIconUI();

        const autoRotateToggle = document.getElementById('auto-rotate');
        if (autoRotateToggle) autoRotateToggle.checked = !!app.autoRotate;

        if (fluidGpuToggle) {
            fluidGpuToggle.checked = !!(app.settings && app.settings.fluidGPU);
            fluidGpuToggle.dispatchEvent(new Event('change'));
        }

        if (venomModeToggle) venomModeToggle.checked = !!app.settings.venomMode;
        const venomPct = Math.round(clamp01(app.settings.venomStrength ?? 0.7) * 100);
        if (venomStrengthSlider) {
            venomStrengthSlider.value = String(venomPct);
            venomStrengthSlider.disabled = !app.settings.venomMode;
        }
        if (typeof applyVenomStrengthLabel === 'function') {
            applyVenomStrengthLabel((venomPct || 0) / 100);
        } else if (venomStrengthValue) {
            venomStrengthValue.textContent = `${venomPct}%`;
        }

        if (magnetEnabledToggle) magnetEnabledToggle.checked = !!(app.magnetTool && app.magnetTool.enabled);

        if (typeof applyTransitionStyleUI === 'function') {
            applyTransitionStyleUI(app.settings.transitionStyle);
        }

        const dissolveToggle = document.getElementById('dissolve-cycle');
        if (dissolveToggle) dissolveToggle.checked = !!app.settings.dissolveCycle;
        const cycleSecondsSlider = document.getElementById('cycle-seconds');
        if (cycleSecondsSlider) cycleSecondsSlider.value = String(app.settings.cycleSeconds);
        const holdSecondsSlider = document.getElementById('hold-seconds');
        if (holdSecondsSlider) holdSecondsSlider.value = String(app.settings.holdSeconds);
        const chaosSlider = document.getElementById('chaos');
        const chaosPct = Math.round((Number(app.settings.chaos) || 0) * 100);
        if (chaosSlider) chaosSlider.value = String(chaosPct);
        const cycleValue = document.getElementById('cycle-value');
        if (cycleValue) cycleValue.textContent = String(app.settings.cycleSeconds) + 's';
        const holdValue = document.getElementById('hold-value');
        if (holdValue) holdValue.textContent = String(app.settings.holdSeconds) + 's';
        const chaosValue = document.getElementById('chaos-value');
        if (chaosValue) chaosValue.textContent = chaosPct + '%';

        const colorMode = String(app.settings.colorMode || 'original');
        document.querySelectorAll('.color-btn[data-color]').forEach(b => b.classList.remove('active'));
        const activeColorBtn = document.querySelector(`.color-btn[data-color="${colorMode}"]`);
        if (activeColorBtn) activeColorBtn.classList.add('active');
        const particleColorInput = document.getElementById('particle-color-custom');
        const isHex = /^#([0-9a-f]{3}){1,2}$/i.test(colorMode);
        if (particleColorInput && isHex && !activeColorBtn) {
            particleColorInput.value = colorMode;
        }

        const realColorsToggle = document.getElementById('real-colors');
        if (realColorsToggle) realColorsToggle.checked = !!app.settings.realColors;

        const chromaStrengthSlider = document.getElementById('chromatic-strength');
        const chromaStrengthValue = document.getElementById('chromatic-strength-value');
        const chromaPct = Math.round(clamp01(app.settings.chromaticShift ?? 0.18) * 100);
        if (chromaStrengthSlider) chromaStrengthSlider.value = String(chromaPct);
        if (chromaStrengthValue) chromaStrengthValue.textContent = `${chromaPct}%`;
        if (typeof setChromaticControlsDisabled === 'function') {
            setChromaticControlsDisabled(String(colorMode || '') !== 'chromatic');
        } else if (chromaStrengthSlider) {
            chromaStrengthSlider.disabled = String(colorMode || '') !== 'chromatic';
        }

        if (gradientEnabledToggle) gradientEnabledToggle.checked = !!app.settings.gradientOverlayEnabled;
        if (gradientColorAInput) gradientColorAInput.value = app.settings.gradientColorA || '#00d4ff';
        if (gradientColorBInput) gradientColorBInput.value = app.settings.gradientColorB || '#a855f7';
        if (gradientStrengthSlider) gradientStrengthSlider.value = String(Math.round(clamp01(app.settings.gradientStrength) * 100));
        if (typeof applyGradientStrengthLabel === 'function') {
            applyGradientStrengthLabel(app.settings.gradientStrength);
        } else if (gradientStrengthValue) {
            gradientStrengthValue.textContent = `${Math.round(clamp01(app.settings.gradientStrength) * 100)}%`;
        }
        if (gradientDirectionSelect) gradientDirectionSelect.value = app.settings.gradientDirection || 'diag';
        const disableGradient = !(app.settings && app.settings.gradientOverlayEnabled);
        ...
        if (bgButtons && bgButtons.length) {
            bgButtons.forEach(b => b.classList.remove('active'));
            if (app.settings.backgroundMode === 'custom') {
                const bgColor = String(app.settings.backgroundColor || '#0a0a0f');
                const matchBtn = document.querySelector(`.bg-color-btn[data-bg="${bgColor}"]`);
                ...
                if (bgColorInput) bgColorInput.value = String(app.settings.backgroundColor || '#0a0a0f');
            }
        }
    }
    ...
    // Export Settings button (save config as JSON)
    on(document.getElementById('export-settings-btn'), 'click', () => {
        const json = app.exportSettings();
        const blob = new Blob([json], { type: 'application/json' });
        if (typeof app.downloadBlob === 'function') {
            app.downloadBlob(blob, 'particle-settings.json');
            return;
        }
    ...
    if (importSettingsBtn && importSettingsInput) {
        on(importSettingsBtn, 'click', () => {
            importSettingsInput.click();
        });
        on(importSettingsInput, 'change', async () => {
            const file = importSettingsInput.files && importSettingsInput.files[0];
            if (!file) return;
            try {
                const text = await file.text();
                const parsed = JSON.parse(text);
                const ok = typeof app.importSettings === 'function' ? app.importSettings(parsed) : false;
                if (!ok) {
                    alert('That file does not look like a valid settings export.');
                    return;
                }
                syncSettingsUI();
            } catch (err) {
                console.error('Failed to import settings.', err);
                alert('Failed to import settings. Please check the JSON file and try again.');
            } finally {
                importSettingsInput.value = '';
            }
        });
    }
    on(window, 'settings-imported', syncSettingsUI);
