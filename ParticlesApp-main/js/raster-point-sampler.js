...
const DEFAULT_DEPTH_RANGE = 0.45;
const DEFAULT_DEPTH_POWER = 1.1;
const DEFAULT_DEPTH_JITTER = 0.08;
...
export class RasterPointSampler {
    ...
    async sampleImagePacked(imageSource, count, {
        ...
        lumaWeightPower = 1.1,
        intensityPower = 1.0,
        depthMapEnabled = true,
        depthMapStrength = 1.0,
        depthMapInvert = true,
        depthMapPower = DEFAULT_DEPTH_POWER,
        depthMapJitter = DEFAULT_DEPTH_JITTER,
        edgeAuraEnabled = false,
        edgeAuraAmount = 0.12,
        edgeAuraSpread = 0.12,
        edgeAuraOutlier = 0.05
        ...
        const depthSettings = {
            mode: depthMapEnabled ? 'luma' : 'random',
            strength: clamp01(depthMapStrength),
            invert: !!depthMapInvert,
            power: Number.isFinite(depthMapPower) ? depthMapPower : DEFAULT_DEPTH_POWER,
            jitter: clamp01(depthMapJitter),
            range: DEFAULT_DEPTH_RANGE
        };
...
function resolveDepthForPixel({ rgba, pixelIndex, depthSettings, rng }) {
    const baseRange = depthSettings && Number.isFinite(depthSettings.range) ? depthSettings.range : DEFAULT_DEPTH_RANGE;
    const useLuma = !!(depthSettings && depthSettings.mode === 'luma' && rgba);
    if (!useLuma) {
        return (rng() * 2 - 1) * baseRange;
    }

    const strength = clamp01(depthSettings.strength ?? 1);
    const range = baseRange * strength;
    if (range <= 0) return 0;

    const ci = pixelIndex * 4;
    const r = rgba[ci + 0] | 0;
    const g = rgba[ci + 1] | 0;
    const b = rgba[ci + 2] | 0;
    ...
    let depthN = Math.pow(luma, Number.isFinite(depthSettings.power) ? depthSettings.power : DEFAULT_DEPTH_POWER);
    if (depthSettings.invert) depthN = 1 - depthN;

    let z = (depthN * 2 - 1) * range;
    const jitter = clamp01(depthSettings.jitter ?? DEFAULT_DEPTH_JITTER);
    if (jitter > 0) {
        z += (rng() * 2 - 1) * range * jitter;
        if (z > range) z = range;
...
function rejectionSampleMask({
    ...
    while (outIdx < target && attempts < maxAttempts) {
        ...
        data[ox + 2] = resolveDepthForPixel({ rgba, pixelIndex: i, depthSettings, rng });
...
function sampleEdgeAura({
    ...
    while (outIdx < target && attempts < maxAttempts) {
        ...
        data[ox + 2] = resolveDepthForPixel({ rgba, pixelIndex: edgeIdx, depthSettings, rng });
        data[ox + 3] = 1.0;
